...
and, for every "buy now" signal:
  1. Sends an alert email via Gmail SMTP
  2. Logs a row to the Notion "Memecoin Buy Signals" database

Required env vars:
  GMAIL_SENDER        Gmail address used to send (must have an App Password)
  GMAIL_APP_PASSWORD  Gmail App Password (16-char, not your login password)
  NOTION_API_KEY      Notion integration token  (secret_...)

Usage:
  python3 check_signals.py | python3 notify.py
"""
...
def _log_notion(token: dict) -> None:
    ...
    payload = {
        ...
        "properties": {
            ...
            "1h Change %":    {"rich_text": [{"text": {"content": str(token.get("change_1h", ""))}}]},
            "6h Change %":    {"rich_text": [{"text": {"content": str(token.get("change_6h", ""))}}]},
            "24h Change %":   {"rich_text": [{"text": {"content": str(token.get("change_24h", ""))}}]},
            "Volume 24h USD": {"rich_text": [{"text": {"content": str(token.get("volume_24h", ""))}}]},
            "Liquidity USD":  {"rich_text": [{"text": {"content": str(token.get("liquidity_usd", ""))}}]},
...
def main():
    ...
    for token in buy_now:
        ...
        try:
            ...
            print(f"  [SKIP email] env var {e} not set")
        except Exception as e:
            print(f"  [EMAIL ERROR] {e}", file=sys.stderr)
        try:
            _log_notion(token)
        except KeyError as e:
            print(f"  [SKIP notion] env var {e} not set")
        except Exception as e:
