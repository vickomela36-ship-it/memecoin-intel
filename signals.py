"""
Buy/sell signal logic for memecoin tokens.
Uses DexScreener API (free, no auth) to fetch live pair data and scores each token.
"""
...
def _score_pair(pair: dict) -> tuple[float, str]:
    """
    Score a DEX pair 0-100 based on:
...
def get_signals_for_tokens(token_addresses: list[str], threshold: float = 70.0) -> list[TokenSignal]:
...
def get_trending_signals(threshold: float = 70.0, limit: int = 20) -> list[TokenSignal]:
    """Scan top trending Solana/ETH memecoin pairs on DexScreener and return buy signals."""
        ...
        try:
            ...
            for pair in pairs[:limit]:
                ...
                if addr in seen:
                    continue
                seen.add(addr)
                sig = _pair_to_signal(pair, threshold)
                results.append(sig)
        except Exception:
            continue
...
def run(watch_tokens: list[str] | None = None, threshold: float = 70.0) -> list[TokenSignal]:
    """Main entry point: returns all signals (filtered or trending)."""
    if watch_tokens:
...
if __name__ == "__main__":
    ...
    signals = run(WATCH_TOKENS or None, BUY_SCORE_THRESHOLD)
    for s in signals:
        print(f"[{s.signal.upper():8}] {s.token:12} ${s.price:.6f}  score={s.score:.0f}  {s.reason}")
