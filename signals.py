...
def _fetch_trending() -> list[dict]:
    """Search for high-volume meme pairs using the free DexScreener search endpoint."""
    ...
    for q in queries:
        ...
        for p in data.get("pairs") or []:
            ...
            if addr and addr not in seen and vol >= 10_000:
                seen.add(addr)
                results.append(p)
    # Sort by 24h volume desc, cap at 20
    results.sort(key=lambda x: float((x.get("volume") or {}).get("h24", 0) or 0), reverse=True)
    return results[:20]
